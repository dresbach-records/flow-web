...
import { verifyCron } from './middleware/verify-cron.js';
...
import { processScheduledContent, processScheduledPublications } from './services/scheduler.service.js';
...
const API_ROUTES = [
  ...
  { method: 'GET', path: '/api/cron/publish' },
] as const;

/** Cria a aplicação Express (sem listen) — compatível com Vercel Serverless e Node persistente. */
export function createApp(): Express {
  ...
  app.get('/health/readiness', async (_req, res) => {
    try {
      const { firestore } = await import('./infrastructure/firebase/firebase-admin.js');
      await Promise.race([
        firestore().collection('_health').limit(1).get(),
        new Promise((_, reject) => setTimeout(() => reject(new Error('FIREBASE_TIMEOUT')), 3000)),
      ]);
      res.json({ status: 'ok', service: 'flow-api', firestore: 'connected' });
    } catch {
      res.status(503).json({ status: 'degraded', service: 'flow-api', firestore: 'unavailable' });
    }
  });
  ...
  // Cron protegido (Vercel Cron): publica conteúdo agendado.
  app.all('/api/cron/publish', verifyCron, async (_req, res) => {
    try {
      const [personaPosts, scheduled] = await Promise.all([
        processScheduledPublications(),
        processScheduledContent(),
      ]);
      res.json({ ok: true, publishedPersonaPosts: personaPosts, publishedScheduled: scheduled });
    } catch (error) {
      const reason = error instanceof Error ? error.message : 'CRON_PUBLISH_FAILED';
      res.status(500).json({ error: 'CRON_PUBLISH_FAILED', reason });
    }
  });

  // 404 JSON (não serve index.html — este é o backend).
  app.use((_req, res) => {
    res.status(404).json({ error: 'NOT_FOUND' });
  });

  // Error handler global (nunca expõe stack em produção).
  app.use((error: unknown, _req: Request, res: Response, _next: NextFunction) => {
    const status = typeof error === 'object' && error !== null && 'status' in error && typeof (error as { status?: unknown }).status === 'number'
      ...
      : 500;
    console.error('Unhandled error', JSON.stringify({ code: 'INTERNAL', status, error: error instanceof Error ? error.message : 'unknown' }));
