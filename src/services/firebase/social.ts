...
export async function hasLiked(postId: string): Promise<boolean> {
  const uid = firebaseAuth.currentUser?.uid;
  return uid ? (await getDoc(doc(firestore, 'posts', postId, 'likes', uid))).exists() : false;
}
...
export async function listComments(postId: string): Promise<CommentRecord[]> {
  const snapshot = await getDocs(query(collection(firestore, 'posts', postId, 'comments'), orderBy('createdAt', 'asc'), limit(100)));
...
export async function toggleFollow(targetUid: string, following: boolean): Promise<void> {
  ...
  const reverse = doc(firestore, 'users', targetUid, 'followers', uid);
  if (following) {
...
export async function toggleSaved(postId: string, saved: boolean): Promise<void> {
  const uid = requireUid();
  const ref = doc(firestore, 'users', uid, 'saved', postId);
